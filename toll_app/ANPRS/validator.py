...
def validate_english(results):
    ...
    for item in results:
        ...
        if len(text) > 4 and text.isalpha() and item[2] >= config.OCR_THRESHOLD:
            state = text
        elif item[2] >= config.OCR_THRESHOLD:
...
def validate_nepali(results):
    ...
    for item in results:
        text = item[1]
        if len(text) > 4 and '-' in text and item[2] >= config.OCR_THRESHOLD:
            state = text
        elif item[2] >= config.OCR_THRESHOLD:
            all_char.extend(list(text))
    ...
    for item in all_char:
        if item in config.NEP_ALPHA_CHAR_LIST:
            text_part += item
        elif item in config.NEP_DIGIT_CHAR_LIST:
            num_part += item
